...
Restore is destructive only after a source preflight: the snapshot's map chains must materialize and every referenced blob (pages, attachments, extras) must resolve through the index before the target is touched — an `--overwrite` restore removes the existing database and its sidecars, and a corrupt repository must fail before that, not after.
