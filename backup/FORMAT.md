...
Restore is destructive only after the source has proven itself, in two layers. A preflight runs before the target is touched: the snapshot's map chains must materialize, every referenced blob (pages, attachments, extras) must resolve through the index, and the extras tree's paths must pass restore's locality, reserved-name, and collision rules. Failures the index cannot reveal — unreadable or corrupt pack bytes — are covered by ordering instead: the database is materialized and page-verified in a staging temp, attachments and extras are then read (each re-deriving its SHA-256) and written, the full restore proof (integrity_check plus the stats comparison, below) runs against the staging temp, and only after all of that are the target's stale SQLite sidecars removed and the temp renamed over the existing database. An `--overwrite` target's live database therefore survives any content or proof failure up to that final swap; content-path derivation and the proof both read the staging temp, never the not-yet-replaced database.
