...
- `jsonv2`: no production Go file in the module imports `encoding/json` (v1),
  ...
  whose Marshal and Unmarshal reach `encoding/json/v2`. Huma's own defaults
  encode with v1 inside the huma package, so a clean import graph alone is
  not enough; without the install, payloads keep v1 semantics (nil slices as
  `null`). Both checks are module-wide; which API a format reaches is not
  tracked.
